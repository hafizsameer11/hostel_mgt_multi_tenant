...
const Destinations = ({ destinations, filterCategories }: DestinationsProps) => {
    ...
    useEffect(() => {
        ...
        if (cityDestinations.length > 4) {
            const maxSlides = Math.max(0, cityDestinations.length - 4);
            const interval = setInterval(() => {
                setCurrentCitySlide((prev) => {
    ...
    }, [cityDestinations.length, destinationSearch, activeFilter]);

    useEffect(() => {
        setCurrentMountainSlide(0);
        if (mountainDestinations.length > 4) {
            const maxSlides = Math.max(0, mountainDestinations.length - 4);
            const interval = setInterval(() => {
                setCurrentMountainSlide((prev) => {
                    const next = (prev + 1) % (maxSlides + 1);
                    return next;
                });
            }, 5000);
    ...
    }, [mountainDestinations.length, destinationSearch, activeFilter]);
                    ...
                    <div className="mb-6">
                        <div className="relative overflow-hidden">
                            <div
                                ...
                                style={{
                                    transform: `translateX(-${currentCitySlide * (100 / 4)}%)`,
                                }}
                            >
                                {cityDestinations.map((destination) => (
                                    <div
                                        key={destination.id}
                                        className="flex-shrink-0 w-full md:w-[23%] text-center"
                                    >
                                        <div className="relative mb-4">
                                            <div className="w-48 h-48 mx-auto rounded-full overflow-hidden shadow-lg hover:shadow-xl transition-shadow">
                                                <img
                    ...
                    <div>
                        <div className="relative overflow-hidden">
                            <div
                                className="flex gap-6 transition-transform duration-500 ease-in-out"
                                style={{
                                    transform: `translateX(-${currentMountainSlide * (100 / 4)}%)`,
                                }}
                            >
